...
if ($stmt->execute()) {
    echo "<script>alert('Ficha de anamnese salva com sucesso!'); window.location.href = './modeloAL.php';</script>";
} else {
    echo "Erro ao salvar a ficha: " . $stmt->error;
}

$query = "UPDATE ficha_anamnese SET respondido = ? WHERE fk_aluno_id_aluno = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("ss", $c, $id_alun);
$stmt->execute();
